<template>
  <q-card class="constrain-mobile border-radius-10px font-monserrat" flat>
    <q-card-section>
      <div class="row">
        <div class="col-5">
        ...
        <div class="col-2 text-center q-mt-sm">
          <q-btn class="bg-secondary text-white border-radius-10px" icon="calendar_today"
            @click="getAllMeeting(); meeting_status = null" push round/>
    ...
    <q-card-section>
      <div class="q-my-lg row flex-center">
        <div class="q-px-xs col-3">
          <q-btn :class="`${meeting_status === 'REVIEW' ? 'bg-primary text-white' : ''} q-px-sm full-width text-capitalize`"
            outline rounded color="primary" label="Review" style="font-size:13px"
        ...
        <div class="q-px-xs col-3">
          <q-btn
            ...
            outline rounded color="negative" label="Rejected" style="font-size:13px"
        ...
        <div class="q-px-xs col-3">
          <q-btn
            ...
            outline rounded color="secondary" label="Approved" style="font-size:13px"
        ...
        <div class="q-px-xs col-3">
          <q-btn
            ...
            outline rounded color="warning" label="Reschedule" style="font-size:13px"
      ...
      <div class="q-my-lg">
        <q-tabs v-model="tab_date" indicator-color="red" active-color="red" :outside-arrows="true"
          :mobile-arrows="true">
          <q-tab v-for="tabs in date_list" class="col-1 q-mx-sm" :active="tab_date === tabs.date" :name="tabs.date"
            :key="tabs.date" @click="getDataMeeting(tabs.date); getMeetingRangeDate()" style="font-size: 10px;">
            <span class="col text-uppercase">{{tabs.day}}</span>
            <span class="col q-mb-md text-h4 text-bold" style="font-size: 16px;">{{tabs.date}}</span>
            <div v-for="(meeting, index) in meeting_range" :key="index">
      ...
      <div class="q-my-lg">
        <q-tab-panels v-model="tab_date" swipeable animated>
            ...
            <div v-if="meeting_list.length > 0">
              <q-card class="row meetingrequest-card q-mb-md border-radius-10px"
                ...
                <q-card-section class="col-8">
                  <div class="column">
                    <div v-if="meeting.topik.length>20" class="col text-secondary">{{ meeting.topik.substring(0,18)+"..." }}</div>
                    <div v-else class="col text-secondary">{{ meeting.topik }}</div>
                    <div v-if="meeting.deskripsi.length>20" class="col text-grey-6">{{ meeting.deskripsi.substring(0,20)+"..." }}</div>
                    <div v-else class="col text-grey-6">{{ meeting.deskripsi.substring(0,18)+"..." }}</div>
                  </div>
                </q-card-section>
      ...
      <q-card style="width: 700px; border-top: 15px solid #116C54">
        ...
        <q-card-actions class="row flex-center">
          <div class="col q-pa-xs">
            <q-btn class="full-width text-capitalize full-width" rounded color="warning" label="Re-schedule" @click="rescheduleModal = true"  />
          </div>
          <div class="col q-pa-xs">
        ...
        <q-card-section>
          ...
          <div class="q-my-md row">
            ...
            <datetime type="date" :phrases="{ ok: 'Continue', cancel: 'Exit' }" class="col-4"
              ...
              v-model='meeting_detail.date' readonly></datetime>
