<template>
  <div class="q-pa-sm font-monserrat">
    ...
    <div class="q-my-sm row">
      ...
      <q-img class="col-4 q-mr-md" alt="Profile Logo" v-if="data.icon" :src="iconFilter.image" width="40px" height="40px" />
      <q-btn class="col-4 bg-grey text-white text-capitalize" label="Choose Icon" size="md" @click="imageModal = true" />
    </div>
    <div class="q-my-md row">
      <span class="col-8 q-mb-sm">Shor or Hide</span>
      <q-toggle
        class="col-2 q-ml-auto"
      ...
      <q-card style="border-top: 5px solid #116C54; width: 500px;">
        ...
        <q-card-section>
          <q-carousel v-model="slide" swipeable animated padding
            control-color="black"
            navigation
            style="max-height: 300px;" class="text-black q-mt-sm">
            <q-carousel-slide v-for="br in dataCeil" :key="br" :name="br" class="column">
              <div class="row">
                <div class="col-3 q-my-sm text-center column" v-for="brd in Shortcut.slice(br*12-12, br*12)" :key="brd" :name="brd">
                  <q-radio class="" style="z-index: 10; margin-left: 50px; margin-top: -20px; position: absolute;"
                  v-model="data.icon" :val="brd.id" color="teal" size="sm" checked-icon="task_alt" unchecked-icon="panorama_fish_eye" />
                  <q-img alt="Profile Logo" width="38px" height="38px" class="self-center" :src="brd.image" />
...
export default {
  ...
  computed: {
    iconFilter () {
      if (this.data.icon) {
        const d = this.Shortcut.filter(d => d.id === this.data.icon)
        return d[0]
      }
  ...
  created () {
    const fetchData = async () => {
      axios.get('/shortcut/icon').then(response => {
  ...
  methods: {
    saveData () {
      ...
      if (this.$refs.titleRef.hasError || this.$refs.linkRef.hasError || !this.data.icon) {
        ...
        axios.post('/shortcut', formData)
          .then(res => {
            if (res.data.status === 'success') {
              this.$q.notify({
                message: 'Success Add Shortcut',
                position: 'top',
                type: 'positive'
              })
