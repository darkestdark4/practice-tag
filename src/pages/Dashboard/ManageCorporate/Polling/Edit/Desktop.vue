  ...
  <div class="q-pa-sm font-monserrat">
    ...
    <div class="q-my-md column">
      ...
      <div class="q-mb-sm row" v-for="(opt, index) in options" :key="index">
        ...
        <q-btn class="text-red text-capitalize col-lg-4 col-md-2 offset-1" label="Delete" size="sm"
          @click="deleteOption(opt.id, index)" outline :disabled="polling.option === 'swipe'" />
      </div>
      <div class="q-mb-sm row" v-for="(opt, index) in optionsNew" :key="index">
        <q-input outlined v-model="opt.name" placeholder="Option" class="col-lg-9 col-md-9" :disable="polling.option === 'swipe'" />
        <q-btn class="text-red text-capitalize col-lg-4 col-md-2 offset-1" label="Delete" size="sm"
          @click="deleteOptionNew(index)" outline :disabled="polling.option === 'swipe'" />
      </div>
      <q-btn class="text-secondary text-capitalize q-mt-xl" label="Add More Option" size="md" @click="addOption()" :disabled="polling.option === 'swipe'"
        outline rounded />
    </div>
