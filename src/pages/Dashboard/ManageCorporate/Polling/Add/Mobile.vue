    ...
    <div class="q-pa-sm font-monserrat">
        ...
        <div class="q-my-md column">
            ...
            <div class="q-mb-sm row" v-for="(opt, index) in options" :key="index">
                ...
                <q-btn class="text-red text-capitalize col-2 offset-1" label="Delete" size="sm" @click="deleteOption(index)" outline :disabled="polling.option === 'all'" />
            </div>
            <q-btn class="text-secondary text-capitalize q-mt-xl" label="Add More Option" size="md" @click="addOption()" outline rounded :disabled="polling.option === 'all'" />
        </div>
        <div class="q-my-md column">
  ...
  data () {
    return {
      ...
      showOptions: [
        ...
        { label: 'All (Those Open Your Link)', value: 'all' }
      ],
      optionsFn (date) {
