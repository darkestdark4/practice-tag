<template>
    <div class="q-pa-lg font-monserra">
        ...
        <q-separator />
        <div class="q-my-md">
          <span class="q-mt-md text-green-7 cursor-pointer" @click="saveAndGo()">+ Add User</span>
        </div>
        ...
        axios.post('/member/corporate/divisi', data)
          .then(res => {
            if (res.data.status === 'success') {
              ...
              setTimeout(() => this.$router.push({ path: '/dashboard/manage-corporate/divisi/add-user-divisi/' + res.data.data.id }), 1000)
            } else {
              this.$q.notify({
                color: 'negative',
                textColor: 'white',
