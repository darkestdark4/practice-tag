    ...
    <div class="col row q-my-md" v-for="(gr, index) in group" :key="index">
      <q-card class="my-card font-proxima-nova-alt full-width" style="border-right: 5px solid #116C54">
        <q-card-section>
          <div class="row col">
            <div class="col-9">
              <span class="col-10 text-secondary text-bold">{{ gr.divisi_name }}</span> <br>
              <span class="col-10">{{ gr.jumlah_anggota }} User</span>
            </div>
            <div class="col-2 text-right">
              <q-btn color="primary" @click="$router.replace({ path: `/dashboard/manage-corporate/divisi/edit-divisi/`+ gr.id })">
                Manage
              </q-btn>
            </div>
          </div>
