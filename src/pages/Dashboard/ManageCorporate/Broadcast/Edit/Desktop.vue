...
export default {
  ...
  methods: {
      ...
      axios.get('/broadcast/' + this.$route.params.id)
        .then(res => {
          if (res.data.data.image) {
            const image = res.data.data.image.split('/')
            this.broadcast.image = image[6]
          }
    ...
    sendBroadcast () {
      ...
      //   to: this.broadcast.to ? 'corporate-'+this.$store.state.userdata.user.corporate_id+'-'+this.broadcast.to : '',
      ...
      formData.append('to', this.broadcast.to ? 'corporate-' + this.$store.state.userdata.user.corporate_id + '-' + this.broadcast.to : '')
