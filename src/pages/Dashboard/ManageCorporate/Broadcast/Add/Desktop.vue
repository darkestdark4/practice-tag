<template>
    <div class="q-pa-sm font-monserrat">
        ...
        <div class="q-my-md column">
            ...
            <q-file filled bottom-slots v-model="broadcast.image" accept="image/*" label="Upload Here" counter>
        ...
        <div class="q-my-md column">
            ...
            <q-select class="col-3" outlined v-model="broadcast.to" :options="broadcastOption" map-options emit-value />
            <!-- <q-select outlined v-model="broadcast.to" :options="broadcastOption"  emit-value/> -->
        </div>
        <div class="q-my-md column">
            <div class="col row">
...
export default {
  ...
  methods: {
    ...
    sendBroadcast () {
      ...
      if (this.broadcast.to !== '') {
        data.to = 'corporate-' + this.$store.state.userdata.user.corporate_id + '-' + this.broadcast.to
      }
      const formData = new FormData()
      formData.append('image', this.broadcast.image)
      formData.append('type', this.broadcast.type)
      formData.append('title', this.broadcast.title)
      formData.append('description', this.broadcast.description)
      formData.append('show_option', this.broadcast.show_option)
      formData.append('start', this.dateStart + ' ' + this.timeStart)
      formData.append('end', this.dateEnd + ' ' + this.timeEnd)
