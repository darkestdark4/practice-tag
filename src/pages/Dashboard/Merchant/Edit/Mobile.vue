  ...
  <div class="q-pa-sm q-my-xs font-monserrat">
    <div class="row">
      ...
      <div class="col-12 q-my-sm column">
        ...
        <div class="row q-mb-md" v-for="(d, index) in data.file2" :key="d">
          <q-input
            class="col-5"
            v-model="d.nama"
            type="text"
...
export default {
  ...
  methods: {
    getData () {
      this.$axios
        .get(`member/merchant/list/${this.$route.params.id}`)
        .then((response) => {
          if (response.data.status === 'success') {
            this.data.member_id = response.data.data.member_id
            this.data.description = response.data.data.description
            this.data.file2 = response.data.data.file
          } else {
            this.$router.replace({ path: '/dashboard/manage-merchant' })
          }
        })
    },
    getMemberList () {
    ...
    sendData () {
      ...
      this.$axios
        .post(`/member/merchant/${this.$route.params.id}`, formData)
        .then((response) => {
          if (response.data.status === 'success') {
            this.$q.notify({
    ...
    deleteFile (id, index) {
      this.$q
        ...
        .onOk(() => {
          this.$axios
            .delete(`member/merchant/file/${id}`)
            .then((response) => {
              if (response.data.status === 'success') {
                this.data.file2.splice(index, 1)
                this.$axios
                  .get(`member/merchant/list/${this.$route.params.id}`)
                  .then((response) => {
                    if (response.data.status === 'success') {
