...
export default {
  ...
  mounted () {
    this.$store.dispatch('style/submitTitlePage', 'Edit Banner')
    this.getData()
  },
  methods: {
    saveData () {
      ...
      axios
        .post('/banner/' + this.$route.params.id, formData)
        .then((res) => {
    ...
    getData () {
      axios
        ...
        .then((res) => {
          ...
          this.data.title = res.data.data.banner_name
        })
        .catch((err) => {
          console.log(err)
        })
...
<template>
  <div class="q-pa-sm font-monserrat">
    ...
    <q-dialog v-model="bannerModal" style="width: 1200px" persistent>
