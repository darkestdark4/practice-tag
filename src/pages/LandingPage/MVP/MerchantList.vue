    ...
    <q-page-container>
      <q-page class="constrain-web-mini font-roboto" style="background: #D9D9D9;">
        ...
        <div class="row q-pa-md items-center text-center">
          <q-card class="col-5 q-px-sm q-mx-auto q-mb-md cursor-pointer border-radius-10px" @click="$router.push(`/landingpage/cabang-list/${d.partner.id}`)"
          v-for="(d, index) in partnerFilter" :key="index">
            <img class="q-py-sm border-radius-10px" :src="d.partner.image" rounded>
            <div class="row q-pb-sm items-center">
              <!-- <div class="col-5 bg-red-14 border-radius-10px text-center text-white">
                {{ d.partner_info }}
              </div> -->
              <div style="font-size: 12px;" :class="d.partner.partner_info != null ? 'col-6 q-ml-sm' : 'col-12'">{{ d.partner.name }}</div>
            </div>
          </q-card>
        </div>
      </q-page>
...
export default {
  ...
  computed: {
    partnerFilter () {
      if (this.search !== '') {
        return this.partnerList.filter((d) => d.name.toLowerCase().includes(this.search.toLowerCase()))
      } else {
  ...
  methods: {
    getPartner () {
      this.$axios
        ...
        .then((response) => {
          this.partnerList = response.data.data.partner
          // response.data.data.partner.map((data) => {
